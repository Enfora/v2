...
def Init_Interface_Settings_manager(self):
    ...
    save_button = CTk.CTkButton(  # Кнопка Сохранить настройки
        ...
        image=CTk.CTkImage(
            light_image=Image.open("./Assets/save.png"),  # путь к иконке
            dark_image=Image.open("./Assets/save.png"),  # та же иконка для темной темы
    ...
    load_button = CTk.CTkButton(  # Кнопка Загрузить настройки
        ...
        image=CTk.CTkImage(
            light_image=Image.open("./Assets/folder.png"),  # путь к иконке
    ...
    # region Артикул
    article_frame = CTk.CTkFrame(setting_frame, fg_color="lightgrey")
    article_frame.pack(side="top", fill="x", pady=5, padx=10)

    # Надпись слева
    article_label = CTk.CTkLabel(
        article_frame, text="Артикул:", font=CTk.CTkFont(size=14)
    )
    article_label.pack(side="left", padx=(0, 10))

    self.article_entry = CTk.CTkEntry(
        article_frame,
        placeholder_text="Артикул",
        height=36,
        width=150,
        font=CTk.CTkFont(size=18),  # Увеличиваем шрифт
        justify="center",
    )
    self.article_entry.pack(side="left")

    # Кнопка генерации веса
    generate_weight_button = CTk.CTkButton(
        article_frame,
        text="Тест",
        width=80,
        height=36,
        ...
        compound="left",  # иконка слева от текста
        command=self.generate_weight_test
    )
    generate_weight_button.pack(side="left", padx=(5, 10))

    # Поле для веса
    self.weight_entry_test = CTk.CTkEntry(
        article_frame,
        placeholder_text="Вес (кг)",
        height=36,
        width=100,
        font=CTk.CTkFont(size=16),
        justify="center",
    )
    self.weight_entry_test.pack(side="left")
    ...
    self.weights_table = CTk.CTkTextbox(
        info_frame,
        height=200,
        font=CTk.CTkFont(family="Consolas", size=14),  # Моноширинный шрифт для таблицы
    )
    ...
    self.photo = CTk.CTkLabel(info_frame, text="", fg_color="transparent")
    self.photo.pack(side="right", fill="none", expand=False, padx=25, pady=25)
